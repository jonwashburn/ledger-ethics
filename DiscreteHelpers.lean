...
lemma discrete_abs_sum_convexity (l : List ℤ) (mean : ℤ)
  ...
  have h_pointwise : ∀ x ∈ l,
    ...
    · -- Case x > mean: adding 1 increases absolute value
      ...
      -- For positive x - mean, we have |x - mean| = x - mean and |x - mean + 1| = x - mean + 1
      -- So |x - mean| ≤ |x - mean + 1| + 1 becomes x - mean ≤ (x - mean + 1) + 1
      -- which is x - mean ≤ x - mean + 2, clearly true
      have h_pos : x - mean ≥ 0 := le_of_lt h
      rw [Int.natAbs_of_nonneg h_pos]
      rw [Int.natAbs_of_nonneg (by linarith : x - mean + 1 ≥ 0)]
      linarith
      ...
      have : natAbs (x - mean) ≤ natAbs (x - mean - 1) + 1 := by
        ...
        · -- x - mean = n (non-negative)
          ...
          · have : n ≥ 1 := Nat.one_le_iff_ne_zero.mpr h'
            simp [Int.natAbs]
            -- We need: n ≤ n + 1 + 1, which is obvious
            exact Nat.le_add_right n 2
        · -- x - mean = -(n+1) (negative)
          simp [natAbs, Int.natAbs]
          -- |-(n+1)| ≤ |-(n+1) - 1| + 1
          -- n+1 ≤ |-(n+2)| + 1 = n+2 + 1
          -- So we need: n + 1 ≤ (n + 2) + 1 = n + 3, which is true
