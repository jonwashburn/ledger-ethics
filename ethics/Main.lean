...
lemma Int.natAbs_lt_natAbs : ∀ {a b : Int}, (a < b ∧ 0 ≤ b) ∨ (a < b ∧ b < 0 ∧ 0 ≤ a) ∨ (-b < a ∧ a < 0) ↔ Int.natAbs a < Int.natAbs b := by
  ...
  · intro h
    ...
    have : (0 ≤ a ∨ a < 0) ∧ (0 ≤ b ∨ b < 0) := ⟨Int.le_total 0 a, Int.le_total 0 b⟩
    cases this.1 with
    | inl ha =>
      cases this.2 with
      | inl hb =>
      ...
      | inr hb =>
        ...
        exact ⟨by linarith [this, hb], hb, ha⟩
    | inr ha =>
      cases this.2 with
      | inl hb =>
        -- a < 0, 0 ≤ b, so |a| = -a, |b| = b
...
theorem moral_realism (s₁ s₂ : MoralState) :
  ...
  · intro h_lower
    ...
    | inr h_neg =>
      ...
      have h1 : κ s₁ < 0 := by
        -- κ is negative when ledger balance is negative
        simp [curvature]
        -- Use the specific context that led to this case
        have h_le : κ s₁ ≤ 0 := h_neg.1; have h_lt : κ s₁ < κ s₂ := h_lower; by_contra h_zero; rw [h_zero] at h_lt; linarith [h_neg.2, h_lt]
      have h2 : κ s₂ < 0 := by
        -- κ is negative when ledger balance is negative
        simp [curvature]
        -- Use the specific context that led to this case
        have h_le2 : κ s₂ ≤ 0 := h_neg.2; have h_lt2 : κ s₁ < κ s₂ := h_lower; have h1_neg : κ s₁ < 0 := h1; linarith [h_le2, h_lt2, h1_neg]
      simp [Int.natAbs_of_neg h1, Int.natAbs_of_neg h2]
      -- Compare the absolute values of the negative curvatures
      linarith
  · intro h_better
    cases h_signs with
...
lemma curvature_determines_goodness_corrected (s₁ s₂ : MoralState) :
  ...
  | inr h_rest =>
    ...
    | inr h_mixed =>
      ...
      simp [Int.natAbs_of_neg h1, Int.natAbs_of_nonneg (by linarith [h2])]
      linarith
...
lemma goodness_determines_curvature (s₁ s₂ : MoralState) :
  ...
  · -- Both negative case
    ...
    have : -κ s₁ = Int.natAbs (κ s₁) := by
      simp [Int.natAbs]
      cases κ s₁ with
      | ofNat n => exfalso; exact h_contra
      | negSucc n => simp
    have : -κ s₂ = Int.natAbs (κ s₂) := by
      simp [Int.natAbs]
      cases κ s₂ with
